...
final class OpenAiAgentService implements AiAgentInterface {
    ...
    public function processTask(TaskDTO $task, array $tools = []): AgentResultDTO {
        ...
        try {
            // ✅ Only include tools params if there are tools registered
            $params = [
                'model'    => $this->model,
                'messages' => $messages,
            ];
            ...
            if (! empty($allTools)) {
                $params['tools']       = $this->formatToolsForApi($allTools);
            ...
            while ($response->choices[0]->finishReason === 'tool_calls') {
                ...
                if (! empty($allTools)) {
                    $loopParams['tools'] = $this->formatToolsForApi($allTools);
                }
    
                $response = $this->client->chat()->create($loopParams);
            }
